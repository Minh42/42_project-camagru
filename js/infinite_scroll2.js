	...
	window.addEventListener("scroll", function() {
		...
		var y = yOffset + window.innerHeight;
	  	if(y >= contentHeight) {
			...
			httpRequest.onreadystatechange = function(data) {
				if (httpRequest.status === 200 && httpRequest.readyState === 4) {
					...
					for (var i = 0; i < array.length; i++) {
						src = array[i]['image_path'];
						src = src.substr(3, src.length);
						myHTML += `
						<div class="gallery">
							<img class="picture" src="${src}" id="${array[i]['photo_id']}" width="612" height="612">
						</div>
						`
					}
			...
			httpRequest.open('POST', '../camagru/app/load_gallery.php', true);
